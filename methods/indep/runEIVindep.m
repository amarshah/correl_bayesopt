%%%%%
% Models objectives as independent GP functions
%%%%%
...
function[ X,Y,pareto_frontier,pareto_volume ] = runEIVindep( objectives, noiseless_objectives, lower, upper, Xinit, nSamples, nIter, xmin, xmax ) 
...
v = sum(v.*v)/(N-1);      %1xR
 amp_samples{1} = 0.85*v;      %1xR
        temp    = 0.5*sqrt(xmax-xmin); %1xD
...
for iter=1:nIter
    ...
    %   optimize EIV                
    neg_acqui_noderiv = @(x) neg_log_EIV_indep_all_cells_all_samples_noderiv( x, X, Y, amp_samples, ls_samples, nvar_samples, Kinv_samples, mean_samples, int_cells );
